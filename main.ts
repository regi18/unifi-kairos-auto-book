...
// TODO fix this. The function doesn't actually work
/**
...
async function login(username: string, password: string): Promise<boolean> {
    ...
    const kairosResponse = await agent.get(
    ...
    const u = finalResponse.text.match(/window.location="(.*)"/)?.[1];
    if (!u) throw new Error('Failed to match response url');
    const c = await agent.get(u);
...
async function getLecturesList(): Promise<Lecture[]> {
    ...
    const lecturesJson = response.text.match(
        /var lezioni_prenotabili = JSON.parse\('(.*)'\)/gi
    )?.[1];
...
(async function () {
    ...
    lecturesList.forEach((l) => {
        if (!l.prenotata && l.prenotabile)
            bookLecture(l.entry_id, config.fiscalCode);
    });
})();
